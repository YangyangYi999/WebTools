	...
	$(document).on('click','.comment',function(event) {
				...
				$.ajax({
					...
					success : function(data) {
						...
						console.log(data);
						$(location).before(
								"<div class='row highLight'><div class='col-sm-1'><b>"
										+ data.user
										+ "</b></div><div class='col-sm-11'>"
...
<c:set var="contextPath" value="${pageContext.request.contextPath}" />
	...
	<div class="thumbnail space" style="width: 70%; margin: 0 auto">
			<div class="row">
				...
				<div class="col-xs-4">${map.content.menuName}</div>
				...
				<div class="row">
					<div class="col-sm-7">
						<c:choose>
							<c:when test="${map.flag}">
								<span><img name="${map.content.contentID}" class="like"
							...
							<c:otherwise>
								<span><img name="${content.contentID}" class="like"
					...
					<div class="col-sm-4">	
						<small><span class="glyphicon glyphicon-time" />
							${map.content.uploadDate}</small><br/>
						<small>created by ${map.createdByUser.username}</small>
					
				</div>
				</div>
				<div class="row">
					<div class="col-xs-4">${map.content.artical}</div>
				</div>
				</div>
